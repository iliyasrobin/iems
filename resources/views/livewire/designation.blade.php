<div>
    <div class="flex flex-col gap-6 p-4">
        ...
        <!-- Dashboard Stats for Designations -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-lg shadow-sm border border-neutral-200 dark:border-neutral-700 p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Total Designations</p>
                        <h3 class="text-xl font-bold text-gray-800 dark:text-white">{{ $designations->total() }}</h3>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-lg shadow-sm border border-neutral-200 dark:border-neutral-700 p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Active Designations</p>
                        <h3 class="text-xl font-bold text-gray-800 dark:text-white">
                            {{ $designations->where('status', 'active')->count() }}
                        </h3>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-lg shadow-sm border border-neutral-200 dark:border-neutral-700 p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Inactive Designations</p>
                        <h3 class="text-xl font-bold text-gray-800 dark:text-white">
                            {{ $designations->where('status', 'inactive')->count() }}
                        </h3>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-neutral-800 rounded-lg border border-neutral-200 dark:border-neutral-700 shadow-sm">
            ...
            <div class="border-b border-neutral-200 dark:border-neutral-700 p-4">
                <div class="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
                    <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Manage Designations</h2>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                        @forelse ($designations as $index => $designation)
                            <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700/50">
                                <td class="px-4 py-3">{{ $designations->firstItem() + $index }}</td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex items-center space-x-2">
                                        <button wire:click="edit({{ $designation->id }})" class="px-3 py-1 bg-indigo-100 text-indigo-700 hover:bg-indigo-200 rounded-full transition-colors duration-150 ease-in-out dark:bg-indigo-900/30 dark:text-indigo-300 dark:hover:bg-indigo-800/50 flex items-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                            Edit
                                        </button>
                                        <button wire:click="delete({{ $designation->id }})" class="px-3 py-1 bg-red-100 text-red-700 hover:bg-red-200 rounded-full transition-colors duration-150 ease-in-out dark:bg-red-900/30 dark:text-red-300 dark:hover:bg-red-800/50 flex items-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center justify-center gap-2">
                                        ...
                                        @if($searchTerm)
                                            <button wire:click="$set('searchTerm', '')" class="text-sm text-indigo-600 hover:underline mt-1">Clear search</button>
                                        @endif
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="relative w-full max-w-md p-6 overflow-hidden transition-all transform bg-white shadow-xl rounded-lg dark:bg-neutral-800 sm:scale-100"
                ...
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-medium text-gray-800 dark:text-white">
                        {{ $updateMode ? 'Edit Designation' : 'Add New Designation' }}
                    </h3>
                ...
                <form wire:submit.prevent="{{ $updateMode ? 'update' : 'store' }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="description" wire:model="description" rows="3"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-neutral-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-neutral-700 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
                            placeholder="Enter designation description (optional)"></textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end pt-2 gap-2">
                        ...
                        <button type="submit"
                            ...
                            {{ $updateMode ? 'Update Designation' : 'Save Designation' }}
                        </button>
                    </div>
                </form>
            </div>
