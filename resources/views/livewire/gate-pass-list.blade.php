<div>
    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-6">
        <div>
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">Gate Passes</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Create and manage your gate passes for equipment or items.
            </p>
        </div>
        <div class="mt-4 sm:mt-0">
            <button wire:click="createNewGatePass"
                        ...
                        <tr
                            ...
                            <td class="px-4 py-3">
                                <div class="inline-flex items-center space-x-2">
                                    <!-- Show Details Button -->
                                    <button type="button" onclick="showDetails('{{ $gatePass->id }}')"
                                    ...
                                    @if ($gatePass->status === 'pending')
                                        <!-- Edit Button -->
                                        <button wire:click="edit({{ $gatePass->id }})"
                                        ...
                                        <!-- Delete Button -->
                                        <button wire:click="confirmDelete({{ $gatePass->id }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-6 text-center text-gray-500 dark:text-gray-400">
                                No gate passes found. Click the "New Gate Pass" button to create one.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div x-data="{ open: @entangle('modalOpen').live }" 
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div class="relative w-full max-w-md p-6 overflow-hidden transition-all transform bg-white shadow-xl rounded-lg dark:bg-neutral-800 sm:scale-100"
                ...
                <!-- Form -->
                <form wire:submit.prevent="saveGatePass" class="space-y-4">
                    <!-- Item Name -->
                    <div>
                    ...
                    <div>
                        <label for="expected_return_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Expected Return Date (Optional)
                        </label>
                        <input type="date" wire:model="expected_return_date" id="expected_return_date" 
                               class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm">
                        @error('expected_return_date') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
    ...
    <!-- JavaScript for gate pass details modal -->
    <script>
        function showDetails(id) {
            // This would normally open a modal with the gate pass details
            alert('Show details for gate pass #' + id + '\n\nIn a real implementation, this would open a modal with the full details of the gate pass.');
        }
    </script>
